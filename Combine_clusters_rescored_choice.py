...
if len(sys.argv)==1:
    ...
    print('usage: %s -N NB_Zdock -score score_name [for cluster initialization] -score score_name2 [for cluster rescoring] '%sys.argv[0])
    sys.exit(1)
...
for prot in lines:
	...
	sor_bclus1=sortCluster(c_clusters1,DD.scores, fn=score_name2)
	my_list1=[c[0].id for c in sor_bclus1][:N]


	c_clusters2=rC(DD.pList,new_ranked_indexes,5, out='dict', stop=500)
	sor_bclus2=sortCluster(c_clusters2,DD.scores, fn=score_name2)
	my_list2=[c[0].id for c in sor_bclus2]
